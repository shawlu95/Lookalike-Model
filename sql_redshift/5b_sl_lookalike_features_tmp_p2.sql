...
CREATE TABLE sb_marketing.sl_lookalike_features_tmp_p2 AS
SELECT 
	a.member_srl
	,ref_dt
	,a.category
...
FROM (
	...
	FROM 
		...
		sb_marketing.sl_lookalike_daily_sale_p2 AS s
	WHERE
		s.dt BETWEEN (SELECT f_dt FROM sb_marketing.sl_lookalike_vars) AND d.ref_dt
		AND member_srl IN (SELECT member_srl FROM sb_marketing.sl_lookalike_active_customers_p2)
) AS a
LEFT JOIN
