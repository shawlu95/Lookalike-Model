...
CREATE TABLE sb_marketing.sl_lookalike_features_tmp_all_cat AS
SELECT 
	a.member_srl
	,ref_dt
	,u.start_dt
...
FROM (
	...
	FROM 
		...
		sb_marketing.sl_lookalike_daily_sale AS s
	WHERE
		s.dt BETWEEN (SELECT f_dt FROM sb_marketing.sl_lookalike_vars) AND d.ref_dt
		AND member_srl IN (SELECT member_srl FROM sb_marketing.sl_lookalike_active_customers)
) AS a
LEFT JOIN
